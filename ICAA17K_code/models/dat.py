...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, num_classes=1000, expansion=4,
        ...
        self.class_head = nn.Linear(216, num_classes)
        self.class_head2 = nn.Linear(216, num_classes)
        self.sg1 = nn.Sigmoid()
        self.sg2 = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        # x = einops.rearrange(x, 'b (g c) -> b g c')
        # x = x.reshape(x.size(0), 32, 32).unsqueeze(dim=1)

        # print(x.shape)  # ([16, 1024])

        x = self.hst_head(x)
        # print(x.shape) # torch.Size([16, 36])

        x = self.hist_feature(x)
        x = einops.rearrange(x, 'b p w -> w p b')
        # print("x",x.shape)
        ...
        x1 = self.class_head(x)
        x1 = self.sg1(x1)
        x2 = self.class_head2(x)
        x2 = self.sg2(x2)
        x = torch.cat([x1, x2], 1)
