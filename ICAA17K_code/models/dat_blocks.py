...
class DAttentionBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        gate = self.ref_gate(x)
        gate = torch.mean(gate, dim=0).unsqueeze(dim=0)
        gate = (einops.rearrange(gate, 'b p h w -> b h w p')*-99999).sigmoid()
        gate = gate.expand(B * self.n_groups, -1, -1, -1)
