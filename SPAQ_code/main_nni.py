...
from dataset import SPAQDataset
...
def create_data_part(opt):
    train_csv_path = os.path.join(opt['path_to_save_csv'], 'spaqTrain.csv')
    test_csv_path = os.path.join(opt['path_to_save_csv'], 'spaqTest.csv')

    train_ds = SPAQDataset(train_csv_path, opt['path_to_images'], if_train=True)
    test_ds = SPAQDataset(test_csv_path, opt['path_to_images'], if_train=False)
...
def start_train(opt):
    ...
    for e in range(opt['num_epoch']):
        ...
        val_loss, tacc, tlcc, tsrcc = validate(opt, model=model, loader=test_loader, criterion=criterion,
                                               writer=writer, global_step=len(test_loader) * e,
                                               name=f"{opt['experiment_dir_name']}_by_batch",
                                               test_or_valid_flag='valid')

        if ((tsrcc > srcc_best or tacc > tacc_best)) and (tsrcc > 0.70):
            srcc_best = tsrcc
            tacc_best = tacc

            model_savetime = 'SPAQ_'
        ...
        nni.report_intermediate_result(
            {'default': tacc, "vsrcc": tsrcc, "val_loss": val_loss})
    nni.report_final_result({'default': tacc, "vsrcc": tsrcc})
    writer.close()
